...
it.skip("read oop html, get reader mode content, output to a file", async () => {
  ...
  for(let img of imgs) {
    ...
    content = content.replace(url0, 'xxx');
    expect(content).toMatch(/xxx/);
  }
...
it.only("down load image, and save to file: https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Octicons-terminal.svg/24px-Octicons-terminal.svg.png", async () => {
